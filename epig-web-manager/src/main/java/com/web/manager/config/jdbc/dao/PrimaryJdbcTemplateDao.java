 ...
 */
@Repository
public class PrimaryJdbcTemplateDao {
	protected static Logger LOGGER = LoggerFactory.getLogger(PrimaryJdbcTemplateDao.class);
	
	@Autowired
	@Qualifier("primaryJdbcTemplate")
	protected PrimaryJdbcTemplate primaryJdbcTemplate;
	...
	public List<Map<String, Object>> queryForMapList(String sql, Object... args){
		LOGGER.info(sql);
		return primaryJdbcTemplate.queryForList(sql, args);
	}
	...
	public Number insert(String sql, Object... args){
		...
		return primaryJdbcTemplate.insert(sql, args);
	}
	...
	public <T> Pagination<T> paginate(String sql, RowMapper<T> rowMapper, int page, int size, Object... args) {
		...
		int total = primaryJdbcTemplate.queryForObject(sqlToCount(sql), Integer.class, args);
		...
		List<T> elements = primaryJdbcTemplate.query(sqlAppendLimit(sql), rowMapper,
