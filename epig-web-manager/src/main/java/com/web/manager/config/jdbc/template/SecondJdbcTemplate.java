 ...
 * <p/>
 */
	public class SecondJdbcTemplate extends JdbcTemplate {
	
		/**
		 * 构造器
		 * @param dataSource
		 */
	public SecondJdbcTemplate(DataSource dataSource) {
		super(dataSource);
    }
		...
		query(sql, newArgPreparedStatementSetter(args), new RowCallbackHandler() {
			...
			public void processRow(ResultSet rs) throws SQLException {
				...
				while (rs.next()) {
					...
					K id = (K) SecondReflectPrimaryKeyReference.getPrimaryValue(v);
					map.put(id, v);
				}
...
class SecondReflectPrimaryKeyReference {
	...
	public static <T> T getPrimaryValue(Object obj) {
		...
		AccessibleObject accessibleObject = SecondReflectPrimaryKeyReference.getPrimaryKey(obj);
